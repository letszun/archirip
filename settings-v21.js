(() => {
  ...
  const CURRENT_BUILD = "2026.08.14";
  const CURRENT_CATALOG = "v18";

  if (document.querySelector("#settingsToggle")) return;

  const style = document.createElement("style");
  style.textContent = `
    /* One centering rule for every circular control. */
    .round-nav,.round-plus,.theme-toggle,.note-add,.add-submit,
    .settings-toggle,.settings-close{
    ...
    .round-nav.right::before{
      top:calc(50% - 2.25px);
      transform:translate(-50%,-50%) rotate(45deg);
    }
    .round-nav.right::after{
      top:calc(50% + 2.25px);
      transform:translate(-50%,-50%) rotate(-45deg);
    }
    .round-nav.left::before{
      top:calc(50% - 2.25px);
      transform:translate(-50%,-50%) rotate(-45deg);
    }
    .round-nav.left::after{
      top:calc(50% + 2.25px);
      transform:translate(-50%,-50%) rotate(45deg);
    }

    .round-plus::before,.round-plus::after,
    .note-add::before,.note-add::after{
      left:50%;top:50%;
      transform:translate(-50%,-50%);
      transform-origin:50% 50%;
    }
    .round-plus::after,.note-add::after{
    ...
    .settings-toggle{
      position:fixed;
      top:clamp(64px,calc(3vw + 46px),80px);
  ...
  toggle.type = "button";
  toggle.innerHTML = '<span class="settings-dots" aria-hidden="true"><span class="settings-dot"></span><span class="settings-dot"></span><span class="settings-dot"></span></span>';
  ...
  panel.innerHTML = `
    ...
    <div class="settings-row">
      <div class="settings-label">현재 버전</div>
      <div class="settings-value">${CURRENT_VERSION}</div>
    </div>
    <div class="settings-row">
      <div class="settings-label">최신 버전</div>
      <div id="settingsLatestVersion" class="settings-value">확인 전</div>
    </div>
    <div class="settings-row">
      <div class="settings-label">상태</div>
      <div id="settingsVersionStatus" class="settings-value settings-status">업데이트 확인 필요</div>
    </div>
    <div class="settings-row">
      <div class="settings-label">빌드</div>
      <div class="settings-value">${CURRENT_BUILD}</div>
    </div>
    <div class="settings-row">
      <div class="settings-label">책 검색</div>
      <div class="settings-value">알라딘 + Open Library · ${CURRENT_CATALOG}</div>
    </div>
