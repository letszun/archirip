(() => {
  ...
  style.textContent = `
    .round-nav,.round-plus,.theme-toggle,.note-add,.add-submit,
    .settings-toggle,.settings-close{box-sizing:border-box}

    .round-nav::before,.round-nav::after{
      content:"";position:absolute;left:50%;width:8px;height:1px;
      background:var(--control-icon);transform-origin:50% 50%;
    }
    ...
    .round-plus::before,.round-plus::after,.note-add::before,.note-add::after{
      left:50%;top:50%;transform:translate(-50%,-50%);transform-origin:50% 50%;
    }
    ...
    .add-submit::before{
      left:50%;top:50%;transform:translate(-50%,-50%) rotate(-45deg);transform-origin:50% 50%;
    }

    .settings-toggle{
      position:fixed;top:clamp(64px,calc(3vw + 46px),80px);right:clamp(16px,3vw,42px);
      width:38px;height:38px;display:grid;place-items:center;padding:0;
      border:1px solid var(--control-border);border-radius:50%;background:var(--control-bg);
      color:var(--control-icon);z-index:45;
      transition:border-color .16s ease,background-color .22s ease,transform .16s ease;
    }
    ...
    .settings-backdrop{
      position:fixed;inset:0;background:rgba(0,0,0,.08);opacity:0;pointer-events:none;
      transition:opacity .18s ease;z-index:78;
    }
    html[data-theme="dark"] .settings-backdrop{background:rgba(0,0,0,.2)}
    .settings-backdrop.open{opacity:1;pointer-events:auto}

    .settings-panel{
      position:fixed;top:clamp(18px,3vw,32px);right:clamp(16px,3vw,42px);
      width:min(310px,calc(100vw - 32px));padding:22px 20px 18px;
      border:1px solid var(--control-border);border-radius:18px;background:var(--bg);color:var(--ink);
      z-index:80;opacity:0;pointer-events:none;transform:translateY(-8px) scale(.985);transform-origin:top right;
      transition:opacity .18s ease,transform .18s ease,background-color .22s ease;
    ...
    .settings-close::before,.settings-close::after{
      content:"";position:absolute;left:50%;top:50%;width:9px;height:1px;background:var(--control-icon);
    }
    .settings-close::before{transform:translate(-50%,-50%) rotate(45deg)}
    .settings-close::after{transform:translate(-50%,-50%) rotate(-45deg)}

    .settings-row{
      display:grid;grid-template-columns:92px 1fr;gap:14px;padding:13px 0;
      border-top:1px solid var(--line);font-size:11px;line-height:1.5;
    }
    ...
    .settings-refresh{
      display:none;width:100%;margin-top:14px;padding:11px 12px;border-radius:999px;
      background:var(--action-bg);color:var(--action-ink);font-size:10px;text-align:center;
    }
  ...
  panel.innerHTML = `
    ...
    <div class="settings-row"><div class="settings-label">책 추가</div><div class="settings-value">직접 등록</div></div>
