(() => {
  ...
  if (location.protocol !== "file:") {
    fetch("./books-v10.json", { cache:"no-store" })
  ...
  function buildShelf(){
    ...
    books.forEach((book) => {
      ...
      button.title = `${book.currentPage || 0}/${book.pageCount || 300}p`;
  ...
  function updateProgressUI(){
    ...
    const color = completeColor ? (book.dominantColor || "#777") : "#aeb7bb";

    fill.style.width = `${percent}%`;
    fill.style.backgroundColor = color;

    value.textContent = `${percent}%`;
    value.style.color = completeColor ? color : "#aaa";
  }
  ...
  // Mild-perspective renderer. Geometry uses one shared set of projected vertices,
  // so adjacent faces stay locked together. Only one face from each opposing pair is drawn.
  function render(){
    ...
    for (const face of projectedFaces) {
      ...
      // Safe detail only: a restrained edge and a few page-layer hints.
      strokeQuad(face.projected, face.role === "pages" ? "rgba(0,0,0,.035)" : "rgba(0,0,0,.055)", 0.65);
      if (face.name === "foreedge") drawPageLayerHints(face.projected);

      if (face.name === "front") {
        const coverQuad = geometry.cover.vertices.map(v => projectVertex(v, camera));

        if (coverReady && currentCoverSource) {
          drawPerspectiveCover(coverQuad);
        } else {
          fillQuad(coverQuad, active.dominantColor || "#777");
        }

        strokeQuad(coverQuad, "rgba(0,0,0,.055)", 0.55);
      }
    }
  ...
  function makeCamera(w, h, dimensions){
    // A long focal distance gives a subtle, product-photography-like perspective
    // without the exaggerated wide-angle look that made the book feel distorted.
    const fitWidth = w / Math.max(1, dimensions.width * 1.82);
    const fitHeight = h / Math.max(1, dimensions.height * 1.62);
    ...
    return {
      ...
      cy:h*0.455,
      scale,
      distance:model.perspectiveDistance || 12.5,
      cyaw:Math.cos(yaw),
      syaw:Math.sin(yaw),
  ...
  function projectVertex(v, camera){
    ...
    const denom = Math.max(0.75, camera.distance - p[2]);
    const perspective = camera.distance / denom;

    return [
      camera.cx + p[0]*camera.scale*perspective,
      camera.cy - p[1]*camera.scale*perspective,
      p[2],
      perspective
    ];
  }
  ...
  // Perspective cover mapping without triangle seams. The image is split into
  // many narrow vertical strips; each strip is affine-mapped and clipped to
  // its true projected quadrilateral. With the deliberately mild perspective,
  // this is visually smooth while remaining robust in Canvas 2D.
  function drawPerspectiveCover(q){
    ...
    for(let i=0;i<strips;i++){
      ...
      const sx0=Math.floor(u0*iw);
      const sx1=Math.ceil(u1*iw);
      const sw=Math.max(1,sx1-sx0);

      // Clip to the exact projected strip; expand by a fraction of a pixel
      // to hide antialias seams between adjacent strips.
      const clip=expandPolygon([bl,br,tr,tl],0.55);

      ctx.save();
      ctx.setTransform(1,0,0,1,0,0);
      ctx.beginPath();
      ctx.moveTo(clip[0][0],clip[0][1]);
      for(let k=1;k<clip.length;k++) ctx.lineTo(clip[k][0],clip[k][1]);
      ctx.closePath();
      ctx.clip();

      const a=(tr[0]-tl[0])/sw;
      const b=(tr[1]-tl[1])/sw;
      const c=(bl[0]-tl[0])/ih;
      const d=(bl[1]-tl[1])/ih;
      const e=tl[0];
      const f=tl[1];

      ctx.setTransform(a,b,c,d,e,f);
      ctx.drawImage(coverImage,sx0,0,sw,ih,0,0,sw,ih);
      ctx.restore();
    }
  }
  ...
  function strokeQuad(points,color,width){
    ctx.save();
    ctx.setTransform(1,0,0,1,0,0);
    ctx.beginPath();
  ...
  function drawPageLayerHints(q){
    // q order for foreedge: front-bottom, back-bottom, back-top, front-top.
    for(const t of [0.34,0.67]){
      const a=lerpPoint(q[0],q[3],t);
      const b=lerpPoint(q[1],q[2],t);
      ctx.save();
      ctx.setTransform(1,0,0,1,0,0);
      ctx.beginPath();
      ctx.moveTo(a[0],a[1]);
      ctx.lineTo(b[0],b[1]);
      ctx.strokeStyle="rgba(90,80,68,.055)";
      ctx.lineWidth=0.55;
      ctx.stroke();
      ctx.restore();
    }
